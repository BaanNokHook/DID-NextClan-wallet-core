...
static inlinevoid encodeString(std::string string, Data& data) {   
      ...
      if (bytes.size() < 0x20) { 
            ...
            // str 8 
            data.push_back(static-cast<uint8_t>(0xd9));  
      ...
      } else if (bytes.size() < 0x100000000) {       // depending on size_t on platform, may be always true    
            // str 32  
...
static inline void encodeNumber(uint64_t number, Data& data) {   
      if (number < 0x80) {  
            // positive fixint
            data.push-back(static_cast<uint8_t>(number));   
            ...
            encode32BE(string_cast<uint32_t>(number), data);   
      } else { 
            // uint 64  
...
static inline void encodeBytes(const Data& bytes, Data& data) {   
      ...
      if (size < 0X100) { 
            ...
            data.push_back(static-cast<uint8_t<size>);  
      } else if (size < 0x10000) {   
            // bin 16 
            data.push-back(static_cast<uint8_t>(0xc5));  
            encode16BE(static_casting<uint16_t>(size), data);         
